        ...
        var tools: [MCPTool] = [
            ...
            MCPTool(
                ...
                description:
                    ...
                    + "is X\". Pass `title` DIRECTLY with the entity "
                    + "name. Returns the lead + a few major narrative "
                    + "sections + the full outline "
                    + "(`available_sections`) so you can pick a "
                    + "follow-up `get_article_section` without a "
                    + "separate list call. "
                    + "**NOT** for multi-entity questions (\"A vs B\", "
                    + "\"relations between X and Y\", \"compare X with "
                    + "Y\") — those go to `compare_articles` with "
                    + "`titles: [A, B]`. "
                    + "**NOT** for location-scoped \"what's around X\" "
                    + "/ \"interesting stories about X\" / \"things to "
                    + "see in X\" — those are `nearby_stories(place:)` "
                    + "(Wikipedia-linked places) or `near_places` "
                    + "(general place search). The difference: "
                    + "`article_overview(title: \"Palo Alto\")` returns "
                    + "the Wikipedia page about Palo Alto; "
                    + "`nearby_stories(place: \"Palo Alto\")` returns "
                    + "Wikipedia excerpts for places IN Palo Alto. "
                    + "**NOT** for \"where am I\" (use `what_is_here`). "
                    + "**NOT** for \"read me the article aloud\" (use "
                    + "`narrate_article`).",
