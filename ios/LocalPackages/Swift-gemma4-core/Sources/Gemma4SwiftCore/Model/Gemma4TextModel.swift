...
public final class Gemma4TextModel: Module, LLMModel, LoRAModel {
    ...
    /// `LLMModel` requirement — chunked prefill.
    ///
    /// Patched locally (2026-04-19): the upstream 0.1.0 implementation
    /// short-circuits to `.tokens(input.text)`, which makes `TokenIterator`
    /// run a *single* forward over the full prompt. On 1800 tokens that
    /// allocates ~3 GB of transient activations in a single step and
    /// overshoots the 6144 MB `increased-memory-limit` cap on iPhone 17
    /// Pro Max. Mirror the default `LLMModel` implementation from
    /// `mlx-swift-lm/Libraries/MLXLLM/LLMModel.swift` so that `windowSize`
    /// (a.k.a. `GenerateParameters.prefillStepSize`) actually chunks the
    /// prefill, flushing intermediate activations between chunks.
    public func prepare(
        ...
        while y.tokens.size > prefillStepSize {
            let chunk = y.tokens[.newAxis, ..<prefillStepSize]
            _ = self(chunk, cache: cache.isEmpty ? nil : cache)
            eval(cache)
            y = y[prefillStepSize...]
        }
