...
extension Gemma4TextAttention {
    ...
    public func callAsFunction(
        ...
        if let donor = sharedKVDonor {
            ...
            // returns quantized tuples `(weight, scales, biases)`, which
            // we feed straight into `quantizedScaledDotProductAttention`
            // to avoid ever materialising a full-precision K/V tensor
            // in the cache itself. ~4× on KV memory — the biggest
            // remaining lever on iPhone per KV_CACHE_COMPRESSION.md.
            //
            // Donor handling: Gemma 4's KV-sharing pattern (full-attention
            // layers 0..14 donate to shared layers 15..34 of the same
            // type) needs the donor's FULL post-update K/V as regular
            // MLXArrays. We dequantize the quantized tuple back to FP16
            // here before returning — the dequantized tensor is a
            // one-step transient (never re-enters the cache), so the
            // steady-state cache memory stays 4× smaller even though
            // a single forward pass briefly materialises the full K/V.
            if let qCache = cache as? QuantizedKVCacheProtocol {
                ...
                let donorKeys = dequantized(
                    qK.0, scales: qK.1, biases: qK.2,
                    groupSize: qCache.groupSize, bits: qCache.bits,
                    mode: qCache.mode)
                let donorValues = dequantized(
                    qV.0, scales: qV.1, biases: qV.2,
                    groupSize: qCache.groupSize, bits: qCache.bits,
                    mode: qCache.mode)
                return (output: oProj(attnOut), keys: donorKeys, values: donorValues)
            }
            if let cache = cache {
