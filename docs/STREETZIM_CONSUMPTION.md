...
| --- | --- | --- |
| `routing-data/graph.bin` | `SZRGGraph.parse` (`swift/Sources/MCPZimKit/SZRGGraph.swift`) | CSR routing graph: node `lat`/`lon`, adjacency offsets, edge arrays (`target`, `distMeters`, `speedKmh`, `geomIdx`, `nameIdx`), name table, per-edge polyline blob. Zero-copy parsed against `Data.withUnsafeBytes`. |
| `routing-data/graph.bin` (**no geoms**) | same | We currently parse with `decodeGeoms: false` (`SZRGGraph.parse(_:decodeGeoms:)`): skips the polyline blob entirely. Saves ~600 MB on country-scale graphs. Route polylines fall back to node-sequence lat/lons. If we ever wire per-edge curvature into the UI we'll flip this back on. |
| `routing-data/graph.bin` presence | `classifyZim` (`ZimReader.swift`) | Triggers `ZimKind.streetzim` classification. |
