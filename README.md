...
and their Wikipedia/StreetZIM files copy straight to the new device over
AWDL / peer-to-peer Wi-Fi, no router or internet needed, with per-chunk
SHA-256 verification, multi-device parallel pull, and resume (powered by the
vendored [LocalSwarm](https://github.com/jasontitus/localswarm) engine,
`ios/LocalPackages/LocalSwarm/`) — and an **in-app download catalog**:
