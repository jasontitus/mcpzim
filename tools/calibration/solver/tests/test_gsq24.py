...
def test_canary_calls_real_gsq_loop_and_verifies_new_diagnostic_checkpoint(tmp_path,monkeypatch):
    ...
    # Resuming at block 2 now requires the completed blocks' archives to be
    # present, because the resume path refuses a snapshot that would silently
    # re-record them as calibrated from their untrained initial candidates. So this
    # payload set has to look like a real post-retention snapshot. `export` names
    # its files relative to the block, so the RTN candidates are re-keyed to those
    # stems rather than copied under their full `model.layers.<n>.` names.
    for layer in (0,1):
        stripped=tmp_path/f'layer-{layer}';stripped.mkdir()
        for path in initial.glob(f'model.layers.{layer}.*.safetensors'):
            shutil.copyfile(path,stripped/path.name.removeprefix(f'model.layers.{layer}.'))
        archive_directory(stripped,source/f'candidate_block_{layer}')
    ...
    cuda=result['cuda']
    # A CPU fixture cannot be real-24GB proof, and must not be labelled as one:
    # `device_type` is what keeps a run from reporting a device it did not use.
    assert cuda['available'] is False and cuda['device_type']=='cpu'
