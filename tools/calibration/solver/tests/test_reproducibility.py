...
def test_policy_seeds_all_rngs_and_enforces_strict_mode(monkeypatch):
    ...
    # This test pins the CUDA policy; auto-detection now selects MPS on Apple
    # Silicon, so the CUDA branch must be requested explicitly.
    monkeypatch.setattr('solver.reproducibility.resolve_device',lambda _:torch.device('cuda'))
