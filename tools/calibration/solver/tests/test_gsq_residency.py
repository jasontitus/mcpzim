...
# Apple Silicon port: MPS is now a supported execution device (see
# solver/device_policy.py). This test previously asserted 'mps' was REJECTED; it
# now asserts MPS is ACCEPTED where the backend exists, while genuinely invalid
# devices still fail. 'meta' and 'not-a-device' are not devices the solver can
# run GSQ on; 'cpu:1' is a CPU with an index, which the residency layout forbids.
@pytest.mark.parametrize('device',['meta','cpu:1','not-a-device'])
...
def test_mps_execution_device_is_accepted():
    assert execution_device(config(BLOCK_CPU_OFFLOAD,'mps'),'cpu')==torch.device('mps')
    with pytest.raises(ValueError,match='must not have an index'):
        execution_device(config(BLOCK_CPU_OFFLOAD,'mps:0'),'cpu')
...
def test_operational_stop_and_cadence_are_not_checkpoint_identity(tmp_path):
    # A per-block sweep sets gsq_max_blocks=1 while a single-process run leaves it
    # unset, and both must resume the same chain, so the stop point and the
    # snapshot cadence are operational. The recipe is not: it stays bound.
    corpus=tmp_path/'corpus';corpus.mkdir();(corpus/'manifest.json').write_text('{}')
    base={'stage':'gsq','corpus':str(corpus),'runtime_sha256':'a'*64,'checkpoint_seconds':120}
    manifest={'model':'Qwen/Qwen3.8-27B','revision':'b'*40}
    frozen=bind_identity({**base,'gsq_max_blocks':1},manifest)
    assert bind_identity(dict(base),manifest)==frozen
    assert bind_identity({**base,'checkpoint_seconds':300},manifest)==frozen
    with pytest.raises(ValueError,match='differs from frozen'):
        bind_identity({**base,'gsq_max_blocks':1,'identity':frozen,'gsq_epochs':2},manifest)
