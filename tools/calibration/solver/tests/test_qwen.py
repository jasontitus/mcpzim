...
def test_original_bf16_loader_does_not_inherit_fp32_meta_dtype():
    from solver.qwen import init_empty_weights, install_original_tensor
...
def test_local_init_empty_weights_replaces_accelerate_for_nested_modules():
    """The port's own meta context must reach nested parameters and buffers."""
    from solver.qwen import init_empty_weights, set_module_tensor_to_device
    with init_empty_weights():
        module=torch.nn.Sequential(torch.nn.Linear(8,4),torch.nn.LayerNorm(4))
    assert all(p.device.type=='meta' for p in module.parameters())
    assert all(b.device.type=='meta' for b in module.buffers())
    weight=torch.randn(4,8).bfloat16()
    set_module_tensor_to_device(module,'0.weight',torch.device('cpu'),value=weight,dtype=torch.bfloat16)
    assert module[0].weight.dtype==torch.bfloat16
    torch.testing.assert_close(module[0].weight,weight,rtol=0,atol=0)
    assert module[0].weight.requires_grad is True, 'existing parameter requires_grad must be preserved'


def test_load_original_has_no_accelerate_dependency():
    """`accelerate` is absent on this machine; loading must not import it."""
    import ast
    import importlib.util
    import solver.qwen as qwen
    assert importlib.util.find_spec('accelerate') is None, 'accelerate unexpectedly installed'
    tree=ast.parse(Path(qwen.__file__).read_text())
    imported=[]
    ...
    assert not [name for name in imported if name=='accelerate' or name.startswith('accelerate.')]
