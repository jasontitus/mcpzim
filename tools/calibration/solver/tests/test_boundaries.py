...
def test_bf16_head_backward_matches_actual_upstream(monkeypatch):
    ...
    # Resolved rather than hardcoded: the container layout was /opt/upstream, but
    # the same pinned source is checked in locally. See solver/upstream_paths.py.
    from solver.upstream_paths import resolve
    source=Path(resolve())/'gsq/src/quantization/gumbel_quantizer_1bit.py'
