...
def smoke_run(model,records,config,output,checkpointer,allow_cpu_test=False):
    ...
    # "Production" here means "a real accelerator and the real byte costs", not
    # "CUDA". MPS is such an accelerator on this machine, so the gate tests the
    # capability. `allow_cpu_test` still exists for genuine CPU test runs, where
    # the byte-cost requirements are also relaxed below.
    if device.type not in ('cuda','mps') and not allow_cpu_test:
        raise ValueError(
            f'Production smoke requires an accelerator (CUDA or MPS); got {device.type!r}'
        )
    ...
    # NOTE: the `cuda_report` / peak-memory assertions further down this module
    # remain CUDA-specific on purpose -- they validate a CUDA memory report, which
    # has no MPS equivalent. See the guard before them.
    ...
    for kind in (('linear_attention','full_attention') if component in ('gsq','both') else ()):
        ...
        _empty_cache(device)
    if component=='gsq':
        result={'status':'completed','smoke':{'passed':device.type in ('cuda','mps'),'model':'Qwen/Qwen3.8-27B',
...
def cuda_report(device):
    """Device diagnostics for the smoke receipt.

    Named for its historical CUDA-only shape because downstream consumers read
    the key, but it now reports whatever device actually ran. On MPS there is no
    allocator peak counter, so `peak_*` is omitted rather than filled with a
    current sample under a peak's name -- the same distinction the run receipt
    makes (`docs/PORT_VALIDATION.md`, finding R5).
    """
    report={'available':device.type in ('cuda','mps'),'device_type':device.type}
    if device.type=='cuda':
        props=torch.cuda.get_device_properties(device)
        report.update(device=props.name,capability=list(torch.cuda.get_device_capability(device)),
                      total_memory_bytes=props.total_memory,
                      peak_allocated_bytes=torch.cuda.max_memory_allocated(device),
                      peak_reserved_bytes=torch.cuda.max_memory_reserved(device))
    elif device.type=='mps':
        import torch
        report.update(device='apple-mps',
                      ...
                      peak_absent_reason='MPS exposes no allocator peak counter')
    return report
