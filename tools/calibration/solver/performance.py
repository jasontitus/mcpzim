...
def measure(output, stage, operation, device=None, **dimensions):
    def synchronize():
        if device is not None:
            # Route through device_policy so MPS timings are synchronized too.
            # MPS executes lazily; without this the measured interval excludes
            # most of the kernel work and every MPS timing is meaningless.
            _sync(device)
    synchronize()
